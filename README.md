...
- `privacy.html`, `contact.html`, `server-status.html` — footer pages. Server Status is public (no login) and shows a live database ping plus which optional integrations (File Storage, Hotel Search, Wordle, Life Sim) have their env vars configured
...
- `api/` — serverless functions (13 total; the project is on Vercel Pro, which doesn't impose the Hobby plan's hard 12-function-per-deployment cap, so this is no longer a hard constraint on new features — collection+item routes still stay merged into one file each out of habit, and shared helpers still live in `lib/` instead of `api/` so they aren't counted as functions themselves). Collection/item routes take the item id as a `?id=` query param (e.g. `/api/users?id=5`) rather than a path segment — Vercel's optional-catch-all filename convention (`[[...id]].js`) is a Next.js-specific routing feature and doesn't reliably work for plain Vercel Serverless Functions, which is what caused the admin user list to silently break after that merge:
  ...
  - `adsb` — public `GET`, backs the homepage's background map: reads the same admin-configured location as `/api/location` and proxies ADSB.lol's `point` lookup for aircraft within range, returning a small trimmed shape (`hex`, `callsign`, `type`, `altitude`, `track`, `lat`, `lon`). Responses are cached in memory for 12 seconds; on an upstream failure it serves the last known-good list (or an empty one) rather than erroring, since the map is designed to keep working with zero aircraft
...
- **ADS-B background map** (`index.html` only): an atmospheric, non-interactive Leaflet map sits behind the login UI, centered on the same admin-configured location as the status bar. Tiles are CARTO's free `dark_nolabels` basemap (no key, no account), further darkened/desaturated with a CSS filter plus a translucent dim layer sandwiched between the tile pane and the marker/tooltip panes — geography reads as barely-there, while aircraft markers and their hover tooltips stay legible on top of it. Aircraft positions come from `/api/adsb` (ADSB.lol, no key, no account) polled every 15 seconds; markers are tracked by ICAO hex and updated/added/removed in place rather than the map or marker set being rebuilt on every poll, with a slow CSS transition so movement reads as a drift rather than a jump. All map interaction is disabled (no drag/zoom/click) so it behaves as a background, not an application; the actual login UI keeps normal pointer events via an explicit override, everything else is click-through so hovering an aircraft works even "through" the wordmark or empty space. If `/api/location` or Leaflet itself fails, the page just keeps its plain dark background with no map; if `/api/adsb` fails, the map keeps running with whatever aircraft it last had (or none) — neither failure touches the Access Portal button.
