...
- `adsb-map.js` — homepage-only: renders the live ADS-B background map behind the login UI on `index.html`. Uses MapLibre GL JS + OpenFreeMap + ADSB.lol only (no key, no account). See "ADS-B background map" in Notes below.
